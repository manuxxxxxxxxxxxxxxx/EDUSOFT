...
<body>
    ...
    <div class="sidebar">
        ...
        <nav>
            ...
            <a href="#"><i class="fas fa-book"></i><span data-i18n="maestro_panel_cursos">Cursos</span> <span class="badge">3</span></a>
            <a href="#"><i class="fas fa-users"></i><span data-i18n="maestro_panel_alumnos">Alumnos</span></a>
            <a href="#"><i class="fas fa-tasks"></i><span data-i18n="maestro_panel_tareas">Tareas</span> <span class="badge">3</span></a>
    ...
    <div class="main-content">
                ...
                <ul id="cursos-lista">
                    <?php if (count($clases) > 0): ?>
                        <?php foreach ($clases as $clase): ?>
                            <li>
                                <a href="../materias/<?php echo $clase['materia']; ?>.php?id_clase=<?php echo $clase['id']; ?>" data-i18n="maestro_panel_link_curso">
                                    <?php echo htmlspecialchars($clase['nombre_clase']); ?> – <?php echo ucfirst($clase['materia']); ?>
                                </a><br>
                                <small data-i18n="maestro_panel_codigo_clase"><strong>Código de clase:</strong> <?php echo htmlspecialchars($clase['codigo_clase']); ?></small>
                            </li>
                        <?php endforeach; ?>
                    <?php else: ?>
                ...
                <ul id="tareas-lista">
                    ...
                    if ($resultadoTareas->num_rows > 0):
                        while ($tarea = $resultadoTareas->fetch_assoc()): ?>
                            <li>
                                <b data-i18n="maestro_panel_tarea_titulo"><?php echo htmlspecialchars($tarea['titulo']); ?></b> 
                                – <span data-i18n="maestro_panel_tarea_materia"><?php echo ucfirst(htmlspecialchars($tarea['materia'])); ?></span> 
                                – <span data-i18n="maestro_panel_tarea_fecha_entrega">Fecha entrega:</span> <?php echo htmlspecialchars($tarea['fecha_entrega']); ?>
                                <form action="../frontend_maestros/eliminar_tarea.php" method="POST" style="display:inline;">
                                    ...
                                    <button type="submit" class="quick-action" style="margin-left:10px;color:red;" data-i18n="maestro_panel_eliminar_tarea">Eliminar</button>
                                </form>
                            </li>
                        <?php endwhile;
                    else: ?>
                        <li data-i18n="maestro_panel_no_tareas">No hay tareas registradas aún.</li>
                    <?php endif;
                    $stmtTareas->close();
                    ?>
                </ul>
                <button class="quick-action" onclick="window.location.href='../frontend_maestros/subir_tarea.php'" data-i18n="maestro_panel_crear_tarea">Crear tarea</button>
            </div>
        </div>
        ...
        <div id="seccion-materiales" class="seccion-panel">
            <div class="section">
                <h3 data-i18n="maestro_panel_titulo_materiales">Materiales
                    <?php if ($clase_nombre && $clase_materia): ?>
                        — <span data-i18n="maestro_panel_nombre_clase"><?= htmlspecialchars($clase_nombre) ?></span> – <span data-i18n="maestro_panel_materia_clase"><?= htmlspecialchars(ucfirst($clase_materia)) ?></span>
                    <?php endif; ?>
                </h3>
                <ul id="lista-materiales">
                    <?php
                    if (!$id_clase) {
                        echo "<li data-i18n='maestro_panel_no_clase_seleccionada'>📂 No hay clase seleccionada.</li>";
                    } else {
                        ...
                        if ($resultado->num_rows > 0) {
                            while ($material = $resultado->fetch_assoc()) {
                                ...
                                echo "<li>";
                                echo "<span data-i18n='maestro_panel_material_titulo'>$titulo</span> — <a href='$ruta' target='_blank' data-i18n='maestro_panel_material_archivo'>$archivo</a> (<span data-i18n='maestro_panel_material_fecha'>Subido el $fecha</span>) ";
                                ...
                                echo "<button type='submit' class='btn btn-danger' onclick='return confirm(\"¿Seguro que deseas eliminar este material?\");' data-i18n='maestro_panel_eliminar_material'>Eliminar</button>";
                                echo "</form>";
                                echo "</li>";
                            }
                        } else {
            ...
            <div class="section">
                ...
                <div class="avisos-timeline">
                    <div class="aviso-card">
                        ...
                        <span data-i18n="maestro_panel_aviso1">Nuevo aviso en Matemáticas 2°A: "Examen el viernes".</span>
                    </div>
                    <div class="aviso-card">
                        <i class="fas fa-bullhorn icon-orange"></i>
                        <span data-i18n="maestro_panel_aviso2">Reunión con padres el 25/06/2025.</span>
                    </div>
                </div>
            </div>
        </div>
